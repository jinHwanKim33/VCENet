...
class OfflineControllerWithSmallRotation(BaseController):
    ...
    def __init__(
            ...
            grid_size=0.25,
            fov=100,
            offline_data_dir='/media/ailab/afbeb78c-82c2-4640-be3f-67053d1554f9/pl_ws/DETR/AI2Thor_offline_data_2.0.2',
            detection_feature_file_name='det_feature_22_cates_detr.hdf5', #'detr_features_22cls.hdf5'
            grid_file_name='grid.json',
            graph_file_name='graph.json',
            metadata_file_name='visible_object_map.json',
            images_file_name='resnet18_featuremap.hdf5',
            depth_file_name='depth.hdf5',  # choi - depth
            debug_mode=True,
            actions=['MoveAhead', 'RotateLeft', 'RotateRight', 'LookUp', 'LookDown'],
            visualize=True,
            local_executable_path=None,
            optimal_action_file_name='optimal_action.json',  ###hwan deafault - none
            optimal_path_file_name='optimal_path.json',  # choi - path
    ):
        ...
        self.optimal_path_file_name = optimal_path_file_name  # choi - path
        self.depth_file_name = depth_file_name  # choi - depth
        self.grid = None
        self.graph = None
        self.metadata = None
        self.images = None
        ...
        # optimal_path_file_name = 'optimal_path.json',
        self.depth = None
    ...
    def start(self):
        if self.visualize:
            ...
            self.controller.step(
                dict(action="Initialize", gridSize=self.grid_size, fieldOfView=self.fov, renderDepthImage=True)
            )
    ...
    def reset(self, scene_name=None):
        ...
        if scene_name != self.scene_name:
            ...
            '''### revised by choi - path ###'''
            with open(
                    os.path.join(
                        self.offline_data_dir, self.scene_name, self.optimal_path_file_name
                    ),
                    'r',
            ) as f:
                self.optimal_path = json.load(f)
            ...
            #print("#####", self.optimal_action_file_name)
    ...
    def step(self, action, raise_for_failure=False):
        ...
        if self.visualize and next_state is not None:
            ...
            viz_event = self.controller.step(
                dict(action="Look", horizon=next_state.horizon, renderDepthImage=True)
            )
        ...
        if next_state is not None:
            ...
            if next_state_key in neighbors:
                ...
                if self.debug_mode and self.visualize:
                    ...
                    # a = viz_event.depth_frame # for test
                    plt.imshow(viz_event.depth_frame)
                    plt.show()
    ...
    def get_image(self):
        # print("self.images[str(self.state)][:]", self.state, self.images)
    ...
    def get_optimal_action(self, target_class):
        self.optimal_action_file_name = 'optimal_action.json'
        # print(self.state)
        # print(target_class)
        # print("optimal action: ", self.optimal_action[str(self.state)][target_class])
        return self.optimal_action[str(self.state)][target_class]

    ### revised by choi - path ###
    def get_optimal_path(self, target_class):
        # print(self.optimal_path[str(self.state)][target_class])
        return self.optimal_path[str(self.state)][target_class]  ## optimal action path(list)

    def get_arbitrary_optimal_path(self, state, target_class):
        # print("get_arbitrary_optimal_path:", self.optimal_path[str(state)][target_class])
        return self.optimal_path[str(state)][target_class]  ## optimal action path(list)
    ### end ###