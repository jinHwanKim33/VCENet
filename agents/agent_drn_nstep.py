...
class ThorAgent:
    ...
    def action(self, model_options, training, test_update):
        ...
        # record the actions those should be used to compute the loss
        # meta_update_step = False
        # if self.optimal_count >= self.count > 0:  # optimal_count - need to revise
        #     self.count -= 1
        #     if self.episode.meta_learning:
        #         meta_update_step = True
        #
        # elif self.count == 0:
        #     if self.episode.meta_learning:
        #         # k = self.episode.states.count(str(self.episode.environment.controller.state))  # frequency
        #         optimal_path = self.environment.controller.get_optimal_path(self.episode.target_object)  # optimal path
        #         Up = len(optimal_path) + len(self.actions)  # optimal actions + past actions
        #         Down = len(optimal_path)  # optimal actions
        #         N = round(Up / Down)  # + k  # N-step
        #
        #         self.optimal_count = N - 1
        #         print("optimal_count =", self.optimal_count)
        #         print("# count ==", self.count)
        #         print("step", len(self.actions))
        #         if self.meta_duplicate_action:
        #             if str(self.episode.environment.controller.state) in self.episode.states:
        #                 # self.N_store.append(N)
        #                 self.count = self.optimal_count
        #                 meta_update_step = True
        #         elif self.meta_failed_action:
        #             if not self.info:
        #                 meta_update_step = True
        ...
        #             meta_update_step = True
        #
        # # print("after", meta_update_step)
        # if meta_update_step:
        #     optimal_action = self.environment.controller.get_optimal_action(self.episode.target_object)
        #     self.meta_learning_actions[self.action_num - 1] = optimal_action

        # meta learning n-step - server2
        meta_update_step = False
        ...
        if self.episode.meta_learning:
            if self.optimal_count >= self.count > 0:  # optimal_count - need to revise
                ...
                if self.meta_duplicate_action:
                    k = self.episode.states.count(str(self.episode.environment.controller.state))  # frequency
                    optimal_path = self.environment.controller.get_optimal_path(
                        self.episode.target_object)  # optimal path
                    Up = len(optimal_path) + len(self.actions)  # optimal actions + past actions
                    Down = len(optimal_path)  # optimal actions
                    N = round(Up / Down) + k  # N-step
                    ...
                    self.count = self.optimal_count
                    meta_update_step = True
            # print("self.optimal_count", self.optimal_count)
            if meta_update_step:
                # print("meta_update_step")
                optimal_action = self.environment.controller.get_optimal_action(self.episode.target_object)
                self.meta_learning_actions[self.action_num - 1] = optimal_action
        ...
        # opt_action = self.environment.controller.get_optimal_action(self.episode.target_object)
        if self.verbose:
        ...
        # self.optimal_actions_record.append(opt_action)
