...
from datasets.constants import DONE_ACTION_INT, AI2THOR_TARGET_CLASSES, DEADLOCK_PENALTY
...
class ThorAgent:
    ...
    def action(self, model_options, training, test_update):
        ...
        # if self.actions:
        #     # if collision
        #     if str(self.episode.environment.controller.state) == self.episode.states[-1]:
        #         if self.actions[-1][0, 0].item() == 0:
        #             print("|_> collision occur")
        #             # pass

        #   reward
        # need to off for test
        if self.rewards:
            if str(self.episode.environment.controller.state) in self.episode.states:
                self.episode.dl_flag = True
                self.rewards[-1] = DEADLOCK_PENALTY
