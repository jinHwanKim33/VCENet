...
from datasets.constants import GOAL_SUCCESS_REWARD, STEP_PENALTY, RECOVERY_REWARD, DUPLICATE_STATE, UNSEEN_STATE, CLOSER_REWARD, DEADLOCK_PENALTY
...
class BasicEpisode(Episode):
    ...
    def __init__(self, args, gpu_id, strict_done=False):
        ...
        # self.closer_reward = args.closer_reward
    ...
    def judge(self, action):
        ...
        if self.environment.controller.state in self.scene_states:
            if action["action"] != DONE:
                ...
                if self.dl_flag:  # previous state was DEADLOCK state.
                    self.dl_flag = False
                    reward = self.get_closer_reward(action["action"])

                # if self.collision_reward:
                #     reward = self.get_collision_reward()
                # if self.closer_reward:
                #     reward += self.get_closer_reward()
        else:
    ...
    def get_closer_reward(self, action):
        reward = STEP_PENALTY
        action_list = ["MoveAhead", "RotateLeft", "RotateRight", "LookUp", "LookDown", 'Done']
        current_opt_action = action_list[self.environment.controller.get_optimal_action(self.target_object)]
        if str(action) == str(current_opt_action):
    ...
    # def get_collision_reward(self):  for collision
    #     topk = 500  # num of pixel
    #     reward = STEP_PENALTY
    #
    #     current_depth = self.current_depth()
    #     current_depth = np.array(current_depth).flatten().tolist()
    #     current_depth = sorted(current_depth)  # smaller pixel value = closer
    #
    #     topk_distance = current_depth[:topk]  # top k (smallest)
    #     distance_avg = sum(topk_distance) / topk  # average of top k pixel
    #
    #     # action fail & smaller than 900.0
    #     if (not self.environment.last_action_success) and distance_avg < DISTANCE_THRESHOLD:
    #         reward = COLLISION_REWARD  # - 0.1
    #
    #     return reward
